...
| GAP-KIMI-A0-003 | P0 | blocked | KIMI Swarm / QA Agent | Tar quarantine package | `/tmp/slash-crm-kimi-intake-tar` | KIMI tar package fails `npm run typecheck` and `npm run build` on `src/pages/Offboarding.tsx(1214,10): error TS1005: '>' expected`. | Fix syntax error and provide passing quality-gate evidence, or authorize Codex salvage before assembly. | `npm run typecheck` and `npm run build` pass in quarantine. |
| GAP-KIMI-A0-004 | P1 | open | KIMI Swarm / QA Agent | Tar quarantine package | `/tmp/slash-crm-kimi-intake-tar` | KIMI tar package fails lint with 68 problems, including 50 errors. | Fix lint errors or provide approved rule-adjustment rationale. | `npm run lint` passes. |
